...
class MLMBatchFunction:
    ...
    def __call__(self, batch):
        ...
        masked_tokens = torch.tensor(
            [self.pad([self.cls_idx] + t + [self.sep_idx], max_len, self.pad_idx) for t in masked_tokens])
...
def train(args):
    ...
    save_mlm_path = "./mlm_model"

    if not os.path.isdir(save_mlm_path):
        os.makedirs(save_mlm_path)
    ...
    for epoch in tqdm(range(args.epochs), desc='epochs'):
        ...
        if val_loss <= best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(save_mlm_path, 'mlm_best_model.bin'))
            torch.save(args, os.path.join(save_mlm_path, 'mlm_training_args.bin'))
